...
@Component({
  selector: 'app-certifications-platforme',
  templateUrl: './certifications-platforme.component.html',
  styleUrls: ['./certifications-platforme.component.css']
})
export class CertificationsPlatformeComponent implements OnInit {
  ...
  private loadData(plateformeId: number): void {
    ...
    this.CertificationService.getByPlateforme(plateformeId).subscribe({
      next: (Certifications) => {
        ...
        this.totalPages = Math.max(1, Math.ceil(this.allCertifications.length / this.pageSize));
        this.currentPage = 1;
        this.updatePage();
        this.loading = false;
      },
      error: () => {
        this.errorMessage = 'Impossible de charger les Certifications pour le moment.';
        this.loading = false;
      }
    });
  }
  ...
  private updatePage(): void {
    ...
    this.paginatedCertifications = this.allCertifications.slice(start, end);
  }
  ...
  toggleFavorite(Certification: Certification, event: Event): void {
